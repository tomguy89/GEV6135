...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    # start와 stop이 포함일 경우
    mot_start = ((start-1)//10+1)*10
    mot_stop = (stop//10+1)*10
    x = torch.arange(mot_start, mot_stop, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    Returns:
        y: Tensor of shape (N, C) and where C = 1 + max(x) is one more than
            the max value in x. The nth row of y is a one-hot-vector
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    y[range(x.shape[0]), x.argmin(dim=1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
